...
TokenPayGUI::TokenPayGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new TokenPayBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("TokenPay") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/tokenpay"));
    setWindowIcon(QIcon(":icons/tokenpay"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/tokenpay/index.html");
#else
    QFile html("/opt/tokenpay/index.html");
#endif
...
void TokenPayGUI::pageLoaded(bool ok)
{
...
void TokenPayGUI::addJavascriptObjects()
{
...
void TokenPayGUI::urlClicked(const QUrl & link)
{
...
void TokenPayGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/tokenpay"), tr("&About TokenPay"), this);
    aboutAction->setToolTip(tr("Show information about TokenPay"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for TokenPay"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/tokenpay"), tr("&Show / Hide"), this);
...
void TokenPayGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void TokenPayGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("TokenPay") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/tokenpay_testnet"));
            setWindowIcon(QIcon(":icons/tokenpay_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/tokenpay_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("TokenPay client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/tokenpay_testnet"));
...
void TokenPayGUI::setWalletModel(WalletModel *walletModel)
{
...
void TokenPayGUI::setMessageModel(MessageModel *messageModel)
{
...
void TokenPayGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("TokenPay client"));
    trayIcon->setIcon(QIcon(":/icons/tokenpay"));
...
#ifndef Q_OS_MAC
void TokenPayGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void TokenPayGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to TokenPay network", "", count));
}

void TokenPayGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void TokenPayGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void TokenPayGUI::changeEvent(QEvent *e)
{
...
void TokenPayGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void TokenPayGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::TPAY, nFeeRequired));
...
void TokenPayGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void TokenPayGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void TokenPayGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void TokenPayGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void TokenPayGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void TokenPayGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid TokenPay address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void TokenPayGUI::handleURI(QString strURI)
{
...
void TokenPayGUI::setEncryptionStatus(int status)
{
...
void TokenPayGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void TokenPayGUI::changePassphrase()
{
...
void TokenPayGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void TokenPayGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void TokenPayGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void TokenPayGUI::updateStakingIcon()
{
...
void TokenPayGUI::detectShutdown()
{
