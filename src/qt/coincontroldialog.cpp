...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog, TokenPayBridge *bridge)
{
    if (!model) return;
    ...
    } else
    {
        ...
        int nDisplayUnit = BitcoinUnits::TPAY;
        ...
        l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : ""); // Change < 0.01TPAY
