#include "tokenpaybridge.h"

#include "tokenpaygui.h"
#include "guiutil.h"
...
TokenPayBridge::TokenPayBridge(TokenPayGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void TokenPayBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void TokenPayBridge::setMessageModel()
{
...
void TokenPayBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void TokenPayBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void TokenPayBridge::populateOptions()
{
...
// Transactions
bool TokenPayBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void TokenPayBridge::clearRecipients()
{
    recipients.clear();
}

bool TokenPayBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 TPAY, 1 TokenPay
        switch(rcp.txnTypeInd)
        {
            case TXT_TPAY_TO_TPAY:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::TPAY, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_TPAY_TO_ANON:
                formatted.append(tr("<b>%1</b> to TOKENPAY %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::TPAY, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b> TOKENPAY, ring size %2 to TOKENPAY %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::TPAY, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_TPAY:
                formatted.append(tr("<b>%1</b> TOKENPAY, ring size %2 to TPAY %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::TPAY, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::TPAY, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsTokenPayBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your tokenpay balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::TPAY, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void TokenPayBridge::openCoinControl()
{
...
void TokenPayBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void TokenPayBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap TokenPayBridge::listAnonOutputs()
{
...
void TokenPayBridge::populateTransactionTable()
{
...
void TokenPayBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void TokenPayBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString TokenPayBridge::transactionDetails(QString txid)
{
...
// Addresses
void TokenPayBridge::populateAddressTable()
{
...
void TokenPayBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void TokenPayBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString TokenPayBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString TokenPayBridge::lastAddressError()
{
    QString sError;
...
QString TokenPayBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void TokenPayBridge::updateAddressLabel(QString address, QString label)
{
...
bool TokenPayBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool TokenPayBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void TokenPayBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void TokenPayBridge::appendMessage(int row)
{
...
void TokenPayBridge::populateMessageTable()
{
...
void TokenPayBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool TokenPayBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool TokenPayBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString TokenPayBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);;
}

bool TokenPayBridge::setPubKey(QString address, QString pubkey)
{
...
bool TokenPayBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
QString TokenPayBridge::createGroupChat(QString label)
{
...
QString TokenPayBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList TokenPayBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
QString TokenPayBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap TokenPayBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap TokenPayBridge::listLatestBlocks()
{
...
QVariantMap TokenPayBridge::findBlock(QString searchID)
{
...
QVariantMap TokenPayBridge::blockDetails(QString blkHash)
{
...
QVariantMap TokenPayBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap TokenPayBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap TokenPayBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap TokenPayBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap TokenPayBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap TokenPayBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap TokenPayBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap TokenPayBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap TokenPayBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with TokenPay prefix.");
            return result;
        }
...
QVariantMap TokenPayBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap TokenPayBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap TokenPayBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
