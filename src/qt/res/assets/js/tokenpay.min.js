...
function addSendAddress() {
  ...
  var g;
  var data;
  if (name = $("#new-send-label").val(), udataCur = $("#new-send-address").val(), g = bridge.getAddressLabel(udataCur), "" !== g) {
  ...
  var camelKey = 0;
  if (data = bridge.newAddress(name, camelKey, udataCur, true), "" === data) {
    var to = bridge.lastAddressError();
...
function addressBookInit() {
  var r20 = [{
    ...
    fun : function() {
      ...
      if (bridge.deleteAddress(target.text())) {
...
function getIconTitle(value) {
  return "unverified" == value ? "fa fa-cross " : "verified" == value ? "fa fa-check " : "contributor" == value ? "fa fa-cog " : "tokenpayteam" == value ? "fa fa-code " : "";
}
...
function openConversation(key, recurring) {
  function convert(src) {
    return micromarkdown.parse(emojione.toImage(src)).replace(/<a class="mmd_tokenpay" href="(.+)">(.+)<\/a>/g, '<a class="mmd_tokenpay" onclick="return confirmConversationOpenLink()" target="_blank" href="$1" data-title="$1">$1</a>');
  }
  if (recurring) {
...
function createGroupChat() {
  ...
  var camelKey = bridge.createGroupChat(key);
...
function acceptInvite(key, data, endpoint) {
  ...
  var camelKey = bridge.joinGroupChat(key, data);
...
function inviteGroupChat(data) {
  ...
  var result = [];
  if (out.length > 0) {
    result = bridge.inviteGroupChat(data, out, $("#message-from-address").val());
  }
  result.length > 0;
}
...
function openInviteModal() {
  ...
  var display_id = bridge.getAddressLabel(current_key).replace("group_", "");
...
function deleteMessages(key, id) {
  ...
  for (;i < meta.messages.length;i++) {
    if (void 0 === id) {
      if (!bridge.deleteMessage(meta.messages[i].id)) {
        return false;
      }
...
function dumpStrings() {
  function clone(dataAndEvents) {
    return'QT_TRANSLATE_NOOP("TokenPayBridge", "' + dataAndEvents + '"),\n';
  }
  var data = "";
...
var unit = {
  ...
  setType : function(v) {
    switch(this.type = void 0 == v ? 0 : v, v) {
      ...
      case 2:
        this.name = "uTPAY";
        this.display = "&micro;TPAY";
        break;
      case 3:
        this.name = "sTPAY";
        this.display = "tpaytoshi";
        break;
      default:
        this.name = this.display = "TPAY";
    }
...
var overviewPage = {
  init : function() {
    ...
    this.tokenpayBal = $(".tokenpay_balance");
...
var blockExplorerPage = {
  ...
  findBlock : function(value) {
    if ("" === value || null === value) {
      ...
      if (blockExplorerPage.foundBlock = bridge.findBlock(value), "" !== blockExplorerPage.foundBlock.error_msg) {
  ...
  prepareBlockTable : function() {
    $("#latest-blocks-table  > tbody tr").selection().on("click", function() {
      ...
      $("#block-txs-table > tbody tr").selection().on("dblclick", function(dataAndEvents) {
        ...
        selectedTxn = bridge.txnDetails(r20, $(this).attr("data-value").trim());
        ...
        for (value in selectedTxn.transaction_inputs) {
          ...
          tagList.append("<tr data-value=" + cachedObj.input_source_address + ">                                                   <td>" + cachedObj.input_source_address + "</td>                                                   <td>" + cachedObj.input_value + "</td>                                                </tr>");
        }
        ...
        for (value in selectedTxn.transaction_outputs) {
          ...
          $options.append("<tr data-value=" + style.output_source_address + ">                                                 <td>" + style.output_source_address + "</td>                                                 <td>" + style.output_value + "</td>                                            </tr>");
        }
...
var walletManagementPage = {
  ...
  newMnemonic : function() {
    var c = bridge.getNewMnemonic($("#new-account-passphrase").val(), $("#new-account-language").val());
    var g = c.error_msg;
    var i = c.mnemonic;
    if ("" !== g) {
      alert(g);
  ...
  recoverKey : function() {
    return result = bridge.importFromMnemonic($("#recover-key-mnemonic").val().trim(), $("#recover-passphrase").val().trim(), $("#recover-account-label").val().trim(), $("#recover-bip44").prop("checked"), 1443657600), "" === result.error_msg || (alert(result.error_msg), false);
  },
  setMaster : function() {
