...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedTokenPayBal(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0),
...
qint64 WalletModel::getTokenPayBalance() const
{
    return wallet->GetTokenPayBalance();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    qint64 newTokenPayBal = getTokenPayBalance();
    qint64 newStake = getStake();
    qint64 newUnconfirmedBalance = getUnconfirmedBalance();
    qint64 newImmatureBalance = getImmatureBalance();

    if (cachedBalance != newBalance
        || cachedTokenPayBal != newTokenPayBal
        ...
        || cachedImmatureBalance != newImmatureBalance)
    {
        ...
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedImmatureBalance = newImmatureBalance;
        emit balanceChanged(newBalance, newTokenPayBal, newStake, newUnconfirmedBalance, newImmatureBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("TokenPay is not connected!"));
    ...
    {
        nBalance = wallet->GetTokenPayBalance();
        if ((nTotalOut + MIN_TX_FEE_ANON) > nBalance)
            return SendCoinsReturn(SCR_AmountWithFeeExceedsTokenPayBalance, MIN_TX_FEE_ANON);
    };

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.txnTypeInd == TXT_TPAY_TO_TPAY
                || rcp.txnTypeInd == TXT_ANON_TO_TPAY)
            {
                // -- out spec
                std::string sError;
...
void WalletModel::emitBalanceChanged(qint64 balance, qint64 tokenpayBal, qint64 stake, qint64 unconfirmedBalance, qint64 immatureBalance) { emit balanceChanged(balance, tokenpayBal, stake, unconfirmedBalance, immatureBalance); }
