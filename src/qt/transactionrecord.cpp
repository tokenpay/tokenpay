...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return TokenPayGUI::tr("Received with");
    case RecvFromOther:
        return TokenPayGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return TokenPayGUI::tr("Sent to");
    case SendToSelf:
        return TokenPayGUI::tr("Payment to yourself");
    case Generated:
        return TokenPayGUI::tr("Mined");
    case RecvTokenPay:
        return TokenPayGUI::tr("Received tokenpay");
    case SendTokenPay:
        return TokenPayGUI::tr("Sent tokenpay");
    case Other:
        return TokenPayGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvTokenPay:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendTokenPay:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredTPAY, nCredTokenPay;
    wtx.GetCredit(nCredTPAY, nCredTokenPay, true);
    int64_t nCredit = nCredTPAY + nCredTokenPay;
    ...
    {
        if (nNet > 0 && nCredTokenPay > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvTokenPay, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendTokenPay, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvTokenPay;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv tokenpay
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendTokenPay;
                    sub.address = CBitcoinAddress(ckidD).ToString();
