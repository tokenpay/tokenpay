...
int nCoinbaseMaturity = 100;		// 288 blocks depth for newly generated coins
int nStakeMinConfirmations = 100;	// 288 blocks depth for minted coins
...
const string strMessageMagic = "TokenPay Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        LogPrintf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TokenPay", CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
