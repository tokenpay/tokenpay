...
int64_t CWallet::GetTokenPayDebit(const CTxIn& txin) const
{
    if (!txin.IsAnonInput())
        return 0;

    // - amount of owned tokenpay decreased
...
int64_t CWallet::GetTokenPayCredit(const CTxOut& txout) const
{
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s TPAY %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                };
            } else
...

    {
        ...
        for (WalletTxMap::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsTrusted() && pcoin->nVersion == ANON_TXN_VERSION)
                nTotal += pcoin->GetAvailableTokenPayCredit();
        };
    }
...
bool CWallet::SendTpayToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    if (fDebugRingSig)
        LogPrintf("SendTpayToAnon()\n");
    ...
    if (vNodes.empty())
    {
        sError = _("Error: TokenPay is not connected!");
        return false;
    };
...
bool CWallet::SendAnonToAnon(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    if (nValue + nTransactionFee > GetTokenPayBalance())
    {
        sError = "Insufficient TokenPay funds";
        return false;
    };
...
bool CWallet::SendAnonToTpay(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    if (fDebug)
        LogPrintf("SendAnonToTpay()\n");
    ...
    if (!AddAnonInputs(nRingSize == 1 ? RING_SIG_1 : RING_SIG_2, nValue, nRingSize, vecSend, vecChange, wtxNew, nFeeRequired, false, sError2))
    {
        LogPrintf("SendAnonToTpay() AddAnonInputs failed %s.\n", sError2.c_str());
        sError = "AddAnonInputs() failed: " + sError2;
        return false;
    };
