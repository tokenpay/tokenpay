...
public:
    ...
    int64_t GetTokenPayBalance() const;
    ...
    bool CreateTransaction(const std::vector<std::pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, int64_t& nFeeRet, int32_t& nChangePos, const CCoinControl *coinControl=NULL, bool fSubtractFeeFromAmount = false);
    bool CreateTransaction(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, int64_t& nFeeRet, const CCoinControl *coinControl=NULL, bool fSubtractFeeFromAmount = false);
    ...
    std::string SendMoney(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee=false, bool fSubtractFeeFromAmount = false);
    std::string SendMoneyToDestination(const CTxDestination& address, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee=false, bool fSubtractFeeFromAmount = false);
    ...
    bool SendTpayToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendAnonToTpay(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetTokenPayCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nTPAY, int64_t& nTokenPay) const
    {
        nTPAY = 0;
        nTokenPay = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nTokenPay += GetTokenPayCredit(txout);
            } else
                nTPAY += GetCredit(txout);
            if (!MoneyRange(nTPAY)
                || !MoneyRange(nTokenPay))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
    };
...
public:
    ...
    mutable bool fAvailableTokenPayCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableTokenPayCreditCached;
    
    mutable int64_t nCredTPAYCached;
    mutable int64_t nCredTokenPayCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableTokenPayCreditCached = false;
        
        nCredTPAYCached = 0;
        nCredTokenPayCached = 0;
        ...
        nAvailableTokenPayCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredTPAY, int64_t& nCredTokenPay, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredTPAY = nCredTokenPay = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredTPAYCached = 0;
            nCredTokenPayCached = 0;
            pwallet->GetCredit(*this, nCredTPAYCached, nCredTokenPayCached);
            fCreditSplitCached = true;
        };
        
        nCredTPAY = nCredTPAYCached;
        nCredTokenPay = nCredTokenPayCached;
        return true;
    }
    ...
    int64_t GetAvailableTokenPayCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableTokenPayCreditCached)
            return nAvailableTokenPayCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableTokenPayCredit() : value out of range");
            };
        };
        
        nAvailableTokenPayCreditCached = nCredit;
        fAvailableTokenPayCreditCached = true;
        return nCredit;
    };
