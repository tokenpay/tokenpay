...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("tokenpaybalance", ValueFromAmount(pwalletMain->GetTokenPayBalance())));
    ...
    if (nNodeMode == NT_FULL)
    {
        ...
        obj.push_back(Pair("tokenpaysupply", ValueFromAmount(pindexBest->nAnonSupply)));
    }
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "Returns a new TokenPay address for receiving payments.  "
...
Value getnewextaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "Returns a new TokenPay ext address for receiving payments."
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "Returns the current TokenPay address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "setaccount <tokenpayaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TokenPay address");
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "signmessage <tokenpayaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage <tokenpayaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress <tokenpayaddress> [minconf=1]\n"
            "Returns the total amount received by <tokenpayaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw std::runtime_error(
            "sendfrom <fromaccount> <totokenpayaddress> <amount> [minconf=1] [comment] [comment-to] [narration] \n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid TokenPay address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        std::string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a TokenPay address or hex-encoded public key\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; TokenPay server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress <tokenpayaddress>\n"
            "Return information about <tokenpayaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw std::runtime_error(
            ...
            "Return information about <tokenpaypubkey>.");
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "Returns a new TokenPay stealth address for receiving payments anonymously."
            + HelpRequiringPassphrase());
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    if (!sxAddr.SetEncoded(sEncoded))
        throw std::runtime_error("Invalid TokenPay stealth address.");
...
Value sendtpaytoanon(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
    ...
    std::string sError;
    if (!pwalletMain->SendTpayToAnon(sxAddr, nAmount, sNarr, wtx, sError))
    {
        LogPrintf("SendTpayToAnon failed %s\n", sError.c_str());
        throw JSONRPCError(RPC_WALLET_ERROR, sError);
    };
    return wtx.GetHash().GetHex();
}
...
Value sendanontotpay(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw std::runtime_error(
    ...
    std::string sError;
    if (!pwalletMain->SendAnonToTpay(sxAddr, nAmount, nRingSize, sNarr, wtx, sError))
    {
        LogPrintf("SendAnonToTpay failed %s\n", sError.c_str());
        throw JSONRPCError(RPC_WALLET_ERROR, sError);
    };
    return wtx.GetHash().GetHex();
}
